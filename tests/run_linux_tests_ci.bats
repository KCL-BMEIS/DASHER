...
@test "Checking XNAT1 war file expanded and exists" {
    file="$storage_path/webapps1/ROOT/favicon.ico"
    [ -f "$file" ]
}

@test "Checking XNAT2 war file expanded and exists" {
    file="$storage_path/webapps2/anon/favicon.ico"
    [ -f "$file" ]
}
...
@test "Checking the xnat user is created and XNAT Rest API works for XNAT 1" {
    response=$(curl -u $xnat_admin_user:$xnat_admin_pwd -X GET 'http://127.0.0.1/data/users')
    [[ $response = *"$xnat_admin_user"* ]]
}
@test "Checking the xnat user is created and XNAT Rest API works for XNAT 2" {
    response=$(curl -u $xnat_admin_user:$xnat_admin_pwd -X GET 'http://127.0.0.1:8082/anon/data/users')
    [[ $response = *"$xnat_admin_user"* ]]
}
...
@test "Deleting test data" {
    HC="$hospital_code"
    response=$(curl -u $xnat_admin_user:$xnat_admin_pwd -X DELETE 'http://127.0.0.1:8082/anon/data/projects/TEST')
    response=$(curl -u $xnat_admin_user:$xnat_admin_pwd -X DELETE 'http://127.0.0.1/data/projects/QUARANTINE/subjects/uploader_S00001')
    response=$(curl -u $xnat_admin_user:$xnat_admin_pwd -X DELETE "http://127.0.0.1:8082/anon/data/projects/RTTQA/subjects/uploader_anon_S00002")
    response=$(curl -u $xnat_admin_user:$xnat_admin_pwd -X DELETE "http://127.0.0.1/data/projects/${HC}/subjects/uploader_S00002")
    directory="$storage_path/archive1/${HC}/arc001/zzsabrSPINE"
    rm -rf "$directory"
    directory="$storage_path/archive2/RTTQA/arc001/TestSession"
    rm -rf "$directory"
    [[ $response != *"error"* ]]
}
